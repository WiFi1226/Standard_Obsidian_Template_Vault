...
tags: {% if tags.length > 0 -%} {% for t in tags -%} {% if "_unread" in t.tag %}-status/unread {% elif "_read" in t.tag %}-status/read {% elif "_reading" in t.tag %}-status/reading {% endif %} {% if "_core" in t.tag %}-important/core {% elif "_uncore" in t.tag %}-important/uncore {% endif %} {% endfor %} {% endif %}
